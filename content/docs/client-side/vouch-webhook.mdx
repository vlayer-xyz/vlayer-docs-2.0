...
This guide shows you how to extract and verify these proofs using the [`POST /api/v2.0/verify`](/server-side/rest-api/verify) endpoint. When a webhook includes more than one proof, send them together to [`POST /api/v2.0/verify-batch`](/server-side/rest-api/verify-batch).
