---
title: Introduction to web proofs
---
...
**vlayer** Web Proofs enable secure integration of Web2 data into blockchain applications. The concept of a Web Proof is straightforward: an external party, called a **Verifier**, can take a transcript of a TLS connection, authenticate its origin (server), and ensure the data hasn't been tampered with. 
...
To preserve privacy and authenticate data origin, TLS connections use symmetric encryption. Symmetric encryption is both secure and fast, but it doesn't help authenticate the data, as the encryption key is shared by both the client and server. This makes it impossible to determine whether the data was provided by the client or the server. Thus, the **Notary** essentially acts as a third party, joining the TLS connection and notarizing its content.
    ...
    <Callout type="info">
    **TLSNotary**: Under the hood, vlayer uses [TLSNotary](https://tlsnotary.org/), an open-source public good project from the Privacy and Scalability Exploration Foundation, sponsored by the Ethereum Foundation. 
    </Callout>
