...
When using the [Vouch platform](https://github.com/introduction) to generate web proofs, you’ll receive webhook responses that include both the proof and the associated data.
...
To receive webhook responses, you need to configure a `webhookUrl` when initiating the verification flow. See the [Getting Started guide](/getting-started/first-steps#webhookurl) or [Integration Example](/examples/integration#step-3-understanding-the-webhook-payload) for details on setting up webhooks.
...
<Callout type="info">
  ...
  privacy. See [redaction configuration](/creating-datasources#redaction) for
  details.
</Callout>
