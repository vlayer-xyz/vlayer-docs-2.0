---
title: POST /verify
description: Verify Web Proof (v2)
---
...
* `version`: TLSN presentation format. Accepted values:
  * `"0.1.0-alpha.12"` — legacy alpha.12 presentations
  * `"2.0.0"` — alpha.15 presentations, covering both MPC and proxy proving modes
* `meta`: Metadata object
* `notaryUrl`: URL of the notary server used during proving. Metadata only — the verifier does not contact the notary. Trust is anchored on the `notaryKeyFingerprint` returned in the response, which is derived from the notary key embedded in the presentation

The endpoint auto-dispatches to the right verifier based on `version`. Alpha.15 MPC and proxy proofs share a single wire format (`"2.0.0"`) and a single verification path — the proving topology is not visible to `/verify`.
...
<Tabs items={['bash', 'javascript']}>
  ...
  <Tab value="javascript">
    ```javascript
    // presentation object from /api/v2.0/prove response
    const presentation = {
      data: "014000000000000000899cdccd31337c96bb9e519aa438ed73cdb47dda5c80e995ef0a1c04bf6c563730cde41724dafc1391b1b81acc5d989a7f7add8...",
      version: "0.1.0-alpha.12",
      meta: {
