...
Verify a previously generated Web Proof. To verify up to 32 proofs in one request, use [`POST /api/v2.0/verify-batch`](/server-side/rest-api/verify-batch). The verification process:
