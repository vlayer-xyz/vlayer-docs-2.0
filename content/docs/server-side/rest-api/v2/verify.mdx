...
Send the presentation object received from `/api/v2.0/prove` directly as the request body. All field names must be in camelCase — unknown fields are rejected with a `400` error.

* `data`: Hex-encoded proof data
* `version`: TLSN protocol version. Must be `"0.1.0-alpha.12"`
