...
v1 could only redact request headers. v2 extends the `redaction` field to also redact request URL query parameters (`request.url_query`), response headers (`response.headers`), and JSON fields in the response body (`response.json_body`). Each field has a matching `*_except` allowlist that reveals only the listed values. See [POST /prove](/server-side/rest-api/prove) for details.
