...
* `presentation`: Web Proof object (direct output from [Web Prover Server `/prove`](/server-side/rest-api/v2/prove))
...
<Callout type="info">
Looking for decode info (types, values, Solidity snippet)? Use [`POST /api/v2.0/debug/evm/journal-decode-helper`](/blockchain/rest-api/v2/journal-decode-helper) with the same request body.
</Callout>
...
<Callout type="info">
**Quick Start**: Call [`/debug/evm/journal-decode-helper`](/blockchain/rest-api/v2/journal-decode-helper) with the same request to get a `solidityCodeSnippet`! It provides ready-to-use Solidity code that you can copy directly into your contract for decoding the journal data.
</Callout>
...
To obtain decoding helpers (types, values, and ready-to-use Solidity snippet), call [`POST /debug/evm/journal-decode-helper`](/blockchain/rest-api/v2/journal-decode-helper) with the same request body.
