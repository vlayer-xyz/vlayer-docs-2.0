...
<Callout type="warn" title="Development Tool Only">
  ...
  [`/compress-web-proof`](/blockchain/rest-api/v2/compress-web-proof) to obtain
  on-chain verifiable ZK proofs for production.
</Callout>

Returns helper data to decode `journalDataAbi` from the ZK Prover's [`/compress-web-proof`](/blockchain/rest-api/v2/compress-web-proof) response.
...
Same as [`/compress-web-proof`](/blockchain/rest-api/v2/compress-web-proof) — pass the Web Proof `presentation` and optional `extraction` config. The endpoint performs a dry run (verification + extraction) and derives decode helper information.
