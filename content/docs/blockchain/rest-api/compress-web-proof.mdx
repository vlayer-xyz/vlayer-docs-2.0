...
<Callout type="info">
Looking for decode info (types, values, Solidity snippet)? Use <code>POST /api/v0/debug/journal-decode-helper</code> with the same request body.
</Callout>
...
To obtain decoding helpers (types, values, and ready-to-use Solidity snippet), call [`POST /debug/journal-decode-helper`](/blockchain/rest-api/journal-decode-helper) with the same request body.
