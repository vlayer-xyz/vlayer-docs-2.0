...
* `presentation`: Web Proof object (direct output from [Web Prover Server `/prove`](/server-side/rest-api/v1/prove))
...
To obtain decoding helpers (types, values, and ready-to-use Solidity snippet), call [`POST /debug/journal-decode-helper`](/blockchain/rest-api/v0/journal-decode-helper) with the same request body.
