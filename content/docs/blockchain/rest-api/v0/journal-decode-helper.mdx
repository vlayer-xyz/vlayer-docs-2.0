...
<Callout type="warn" title="Development Tool Only">
  ...
  [`/compress-web-proof`](/blockchain/rest-api/v0/compress-web-proof) to obtain
...
Same as [`/compress-web-proof`](/blockchain/rest-api/v0/compress-web-proof) — pass the Web Proof `presentation` and optional `extraction` config. The endpoint performs a dry run (verification + extraction) and derives decode helper information.
