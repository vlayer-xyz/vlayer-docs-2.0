...
* [POST /compress-web-proof](/blockchain/rest-api/compress-web-proof) accepts a Web Proof (output of `/prove` from [Web Prover Server](/server-side/rest-api/prove) or payload from vouch webhook), verifies the Web Proof, parses HTTP transcript and returns a ZK proof with ABI-encoded public outputs (journalDataAbi)
* [GET /guest-id](/blockchain/rest-api/guest-id) returns the ID of RISC Zero guest program used to generate ZK Proofs in /compress-web-proof endpoint
